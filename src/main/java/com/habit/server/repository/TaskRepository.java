...
public class TaskRepository {
  ...
  public TaskRepository() { this("jdbc:sqlite:habit.db"); }
  ...
  public void saveTask(Task task, String teamID) {
    try (Connection conn = DriverManager.getConnection(databaseUrl)) {
      String sql =
          "INSERT OR REPLACE INTO tasks (taskId, taskName, description, "
          + "estimatedMinutes, repeatDays, isTeamTask, teamID, dueTime, "
          + "cycleType) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
        ...
        if (task.getRepeatDays() != null && !task.getRepeatDays().isEmpty()) {
          repeatDaysStr = String.join(",", task.getRepeatDays()
                                               .stream()
        ...
        pstmt.setString(
            8, task.getDueTime() != null ? task.getDueTime().toString() : null);
